 ...
 */
export async function getApprenantsOPCOPlanningType(jour, creneau, lieu_id) {
  try {
    // Normaliser le jour en minuscule (BDD stocke en minuscule)
    const jourNormalise = jour.toLowerCase();

    console.log('🔍 DEBUG getApprenantsOPCOPlanningType:', {
      jour,
      jourNormalise,
      creneau,
      lieu_id
    ...
    console.log('👥 Apprenants OPCO trouvés:', apprenants?.length || 0);
...
export async function getEmargementDataOPCO(seanceData) {
  try {
    ...
    // 1. Récupérer les apprenants OPCO prévus au planning type
    const apprenants = await getApprenantsOPCOPlanningType(jour, creneauNormalise, lieu_id);
