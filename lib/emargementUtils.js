...
export async function getEmargementData(seanceData) {
  try {
    ...
    // 2bis. Ajouter les apprenants en presence exceptionnelle pour ce creneau/lieu
    const { data: presencesExceptionnelles } = await supabase
      .from('absences_apprenants')
      .select('apprenant_id, users!absences_apprenants_apprenant_id_fkey(id, nom, prenom, statut_formation)')
    ...
    for (const pe of (presencesExceptionnelles || [])) {
      if (pe.users && !idsExistants.has(pe.users.id)) {
        apprenants.push({
          id: pe.users.id,
          nom: pe.users.nom,
