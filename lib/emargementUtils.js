...
export async function getEmargementData(seanceData) {
  try {
    ...
    // 2bis. Ajouter les apprenants HSP en presence exceptionnelle pour ce creneau/lieu
    // (les non-HSP en presence exceptionnelle vont sur la feuille OPCO, pas ici)
    const { data: presencesExceptionnelles } = await supabase
      .from('absences_apprenants')
      .select('apprenant_id, users!absences_apprenants_apprenant_id_fkey(id, nom, prenom, statut_formation, dispositif)')
      .eq('type', 'presence_exceptionnelle')
      .eq('date_specifique', date)
      .eq('creneau', creneauNormalise)
      .eq('lieu_id', lieu_id)
      .eq('statut', 'actif');

    const idsExistants = new Set(apprenants.map(a => a.id));
    for (const pe of (presencesExceptionnelles || [])) {
      if (pe.users && pe.users.dispositif === 'HSP' && !idsExistants.has(pe.users.id)) {
        apprenants.push({
          id: pe.users.id,
          nom: pe.users.nom,
          prenom: pe.users.prenom,
          statut_formation: pe.users.statut_formation
        });
        idsExistants.add(pe.users.id);
      }
    }
    ...
    const apprenantsAvecStatut = apprenants.map(apprenant => {
      const isAbsent = !!absences[apprenant.id];
      const isSuspendu = apprenant.statut_formation === 'suspendu';

      let commentaire = '';
      if (isAbsent) {
        commentaire = absences[apprenant.id]?.motif ? `ABSENT - ${absences[apprenant.id].motif}` : 'ABSENT';
      } else if (isSuspendu) {
        commentaire = 'Absence - suspension';
      }
      ...
      return {
        ...
        absent: isAbsent,
        motif_absence: isAbsent ? (absences[apprenant.id]?.motif || null) : null,
        commentaire
      };
    });
...
export async function getEmargementDataOPCO(seanceData) {
  try {
    ...
    // 1bis. Ajouter les apprenants OPCO en presence exceptionnelle pour ce creneau/lieu
    // (les HSP en presence exceptionnelle vont sur la feuille HSP, pas ici)
    const { data: presencesExceptionnelles } = await supabase
      .from('absences_apprenants')
      .select('apprenant_id, users!absences_apprenants_apprenant_id_fkey(id, nom, prenom, statut_formation, dispositif, date_entree_formation, date_sortie_previsionnelle)')
      .eq('type', 'presence_exceptionnelle')
      .eq('date_specifique', date)
      .eq('creneau', creneauNormalise)
      .eq('lieu_id', lieu_id)
    ...
    const idsExistants = new Set(apprenants.map(a => a.id));
    for (const pe of (presencesExceptionnelles || [])) {
      if (!pe.users) continue;
      const dispo = pe.users.dispositif;
      // Exclure HSP, CDV et PM (premiere marche) : ils ont leurs propres feuilles
      if (dispo === 'HSP' || dispo === 'CDV' || dispo === 'PM') continue;
      if (idsExistants.has(pe.users.id)) continue;
      apprenants.push({
        id: pe.users.id,
        nom: pe.users.nom,
        prenom: pe.users.prenom,
        statut_formation: pe.users.statut_formation,
        date_entree_formation: pe.users.date_entree_formation,
        date_sortie_previsionnelle: pe.users.date_sortie_previsionnelle
      });
      idsExistants.add(pe.users.id);
    }
      ...
      apprenants.map(async (apprenant) => {
        ...
        return {
          ...
          periode_formation: dateDebut && dateFin
            ...
            : '',
          commentaire: apprenant.statut_formation === 'suspendu' ? 'Absence - suspension' : ''
        };
      })
    );
