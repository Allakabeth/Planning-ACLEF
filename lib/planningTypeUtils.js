...
/**
 * Vérifie si une date/créneau est fermé (jour férié, fermeture, etc.)
 * Reproduit la logique de estJourFerme de planning-coordo
 */
function isDateFermee(dateStr, creneau, fermetures) {
    if (!fermetures || fermetures.length === 0) return false;
 ...
 * EXCLUT les absents et les jours fermés du planning généré
 */
export async function genererPlanningDepuisType(weekDates, absencesFormateurs, absencesApprenants, fermetures) {
    try {
        const [formateursPT, apprenantsPT, apprenantsActifs, suspensionsRes, historiqueLieuxRes, lieuxRes, apprenantsPTDates] = await Promise.all([
            ...
            // Planning type par dates ponctuelles (mode alternatif au hebdo)
            supabase.from('planning_apprenants_dates')
                .select('apprenant_id, date, creneau, lieu_id')
                .in('date', weekDates)
        ]);
        ...
        const planningApprenantsDates = apprenantsPTDates.data || [];
        ...
        planningFormateurs.forEach(pt => {
            ...
            if (isDateFermee(dateStr, creneau, fermetures)) return;
        ...
        // Collecter les lieux des apprenants en mode "dates ponctuelles"
        planningApprenantsDates.forEach(pt => {
            const dayIndex = weekDates.indexOf(pt.date);
            if (dayIndex === -1 || !pt.lieu_id) return;
            const dateStr = weekDates[dayIndex];
            const creneau = creneauVersAffichage[pt.creneau] || pt.creneau;
            if (isDateFermee(dateStr, creneau, fermetures)) return;
            const apprenant = apprenantsMap[pt.apprenant_id];
        ...
        // Remplir apprenants en mode "dates ponctuelles"
        planningApprenantsDates.forEach(pt => {
            const dayIndex = weekDates.indexOf(pt.date);
            if (dayIndex === -1 || !pt.lieu_id) return;
            const dateStr = weekDates[dayIndex];
